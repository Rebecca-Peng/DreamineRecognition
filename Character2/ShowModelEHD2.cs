...
public class ShowModelEHD2 : MonoBehaviour {
	...
	void Update () {
		match = target.GetComponent<ImproEHD2> ().match;
		if (match) {
			if (display) {

				currentNearest [0] = target.GetComponent<ImproEHD2> ().currentNearest;
				currentNearest [1] = target.GetComponent<ImproEHD2> ().currentNearest;
				currentNearest [2] = target.GetComponent<ImproEHD2> ().currentNearest;
				...
				if (MyPrefabInstantiator.found) {
					...
					model.GetComponent<Renderer> ().material.color = ImproEHD2.COLORHSV;
