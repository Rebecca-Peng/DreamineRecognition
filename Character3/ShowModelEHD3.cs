...
public class ShowModelEHD3 : MonoBehaviour {
	...
	void Update () {
		match = target.GetComponent<ImproEHD3> ().match;
		if (match) {
			if (display) {

				currentNearest [0] = target.GetComponent<ImproEHD3> ().currentNearest;
				currentNearest [1] = target.GetComponent<ImproEHD3> ().currentNearest;
				currentNearest [2] = target.GetComponent<ImproEHD3> ().currentNearest;
				...
				if (MyPrefabInstantiator.found) {
					...
					model.GetComponent<Renderer> ().material.color = ImproEHD3.COLORHSV;
